...
class Auth extends CI_Controller
{
	...
	{		
		...
		{
			$data['total'] = $this->User_model->get_user_data(1);
			...
			$data['data'] = $this->User_model->get_user_data();
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			else
			{
				...
				$this->session->set_flashdata('message', 'Wrong login combination');
		...
		else
		{
			...
			$this->data['border'] = (validation_errors()) ? 'border-danger' : '';
	...
	public function activateUser($id, $code = false)
	{
		...
		else
		{
			$data['response'] = array(
                'title' => 'Error',
                'type' => 'error',
                'message' => 'Something is wrong, user has not been activated',                        
            );
		}
	...
	public function getGroupData()
	{
		...
		$data['total'] = $this->User_model->getGroupData(1);
		$data['per_page'] = $this->input->get('per_page');
		$data['current_page'] = $this->input->get('page');
		$data['last_page'] = ceil($data['total']/$data['per_page']);
		$data['from'] = ($data['current_page'] * $data['per_page']) - ($data['per_page'] - 1);
		$data['to'] = $data['per_page'] * $data['current_page'];
		$data['data'] = $this->User_model->getGroupData();
	...
	*/
	...
	public function createGroup() // Ajax CreateGroup combined with editGroup
	{
				...
				if ($this->form_validation->run() === TRUE)
				{
					...
					else
					{
						$this->data['response'] =  array(
							'title' => 'Error',
							'type' => 'error',
							'message' => 'Something is wrong (avoid special chars), group not updated',                        
						);
					}
				}
			}
		...
		else // new group
		{
			...
			else
			{
				...
				$this->data['response'] =  array(
					'title' => 'Error',
					'type' => 'error',
					'message' => 'Error, please check your input, avoid special char',                        
				);
